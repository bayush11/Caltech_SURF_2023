...
In this section, I will discuss the hard drives that contain the weights of the machine learning model that has been trained above. The purpose of the hard drive is essentially to store the hard drive weights because GitHub does not allow for such large files to be stored. With that in mind, the files that are in the repository currently have all the necessary files to train your model except for the backup folder. The backup folder will be the folder that will store the external weights that essentially make up the machine learning model. These weights are also used in detecting and tracking objects for testing purposes. Referencing them will require the use of the hard drive. Within the hard drive, they are stored as "backup_xxx" where xxx represents the specific application used in it (e.g. binary_original represents the machine learning model used to track particles in the original videos taken).
